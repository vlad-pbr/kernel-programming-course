...
This is a user-space program written in C which implements the slow loris attack. The program receives the following command arguments:

```sh
./a.out <ip> <port> <num_connections> <interval>
```

The program spawns `num_connections` amount of worker child processes. Worker process is in charge of:
...
After spawning the workers, the main process waits for messages to arrive to the message queue and prints them out. Upon receiving an interrups, the main process terminates the workers and quits.
